...
import type { DocLine, Client, TemplateId } from "@/types";
...
export default function InvoiceEditorScreen() {
  ...
  const nextInvoiceNumber = useDataStore((state) => state.nextInvoiceNumber);
  ...
  const buildAiDetails = (): string => {
    ...
    if (totalsResult) {
      chunks.push(`Betrag: ${profile.currency} ${totalsResult.totalGross.toFixed(2)}`);
    }
  ...
  const handleGenerateNotes = async () => {
    ...
    try {
      ...
      const text = await requestInvoiceText({
        ...
        type: "invoice",
        details: buildAiDetails() || "Standard Leistungsbeschreibung",
      });
      setNotes(text);
    } catch (error: any) {
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen
        options={{
          headerShown: true,
          title: existingDoc ? "Edit Invoice" : "New Invoice",
          headerLeft: () => (
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.saveButton} onPress={handleSave}>
          <Text style={styles.saveButtonText}>{existingDoc ? "Update Invoice" : "Create Invoice"}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
